...
Flow:
  1. ESP32 publishes tap JSON to MQTT topic onelink/hardware/tap
  2. brain.py broadcasts card_tap event to kiosk browser via WebSocket (ws://127.0.0.1:8765)
  3. Kiosk UI at /kiosk handles pairing, service selection, and payment via Vercel APIs
...
def on_connect(client, userdata, flags, rc, properties=None):
    ...
    log.info("MQTT subscribed: %s", MQTT_TOPIC)
...
def on_message(client, userdata, msg):
    try:
        ...
        if AUTO_PAY:
            ...
            if not ws_clients:
                log.warning(
                    "No kiosk browser connected (open http://127.0.0.1:8080/ or deployed /kiosk)"
                )

    except json.JSONDecodeError:
        log.error("Corrupted JSON from ESP32")
