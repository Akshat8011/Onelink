 ...
 *
 * Vercel serverless handler for ESP32 RFID taps relayed by the Pi gateway.
 * Body: { "cardUid": "A1B2C3D4", "terminalId": "metro_entry" }
 *
 * Also accepts Pi/ESP32 field names: { "uid": "...", "node": "..." }
 *
 * Security: Authorization: Bearer <HARDWARE_TAP_API_KEY>
 */
...
function authorizeGateway(req) {
  ...
  const token = authHeader.startsWith('Bearer ')
    ...
    : req.headers['x-hardware-api-key'];
...
module.exports = async function handler(req, res) {
  ...
  const auth = authorizeGateway(req);
  if (!auth.ok) {
    return jsonResponse(res, { success: false, error: auth.message }, auth.status);
  }

  const parsed = parseBody(req.body);
  if (parsed.error) {
    return jsonResponse(res, { success: false, error: parsed.error }, 400);
  }
  ...
  try {
    ...
    if (!outcome) {
      return jsonResponse(res, { success: false, error: 'Payment could not be completed' }, 500);
    }
    ...
    return jsonResponse(res, { success: false, error: 'Internal server error' }, 500);
  } finally {
