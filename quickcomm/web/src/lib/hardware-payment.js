...
import { TERMINAL_CONFIG, normalizeCardUid } from './kiosk-shared';
import { notifyBackend } from './notify-backend';
...
export async function processHardwareTap(rawCardUid, rawTerminalId) {
  ...
  try {
    ...
    // Push a realtime balance update to the user's mobile webapp so the two
    // stay in sync after a physical tap (kiosk → webapp).
    await notifyBackend({
      userId: user.userId,
